...
const games = [
  ...
  { id:"kakuro", url:playPath(KAKURO), eyebrow:"Cross sums", title:"Kakuro", description:"No sudoku boxes at all—just runs of digits and the totals they add up to, with no repeats inside a run. The combination table does the arithmetic so you can do the thinking.", meta:"900 puzzles · White belt through to 1st Dan", accent:"green", grid:Array(81).fill(0) },
];

/*
 * A real level-1 grid from the Kakuro pack, clues and all: "" is an answer
 * cell, a digit is one already written in, "#" is a plain black square and
 * "across,down" is a clue. Nine wide, so it drops straight into .real-board.
 */
const kakuroBoard = [
  "#","#",",39",",15",",9","#","#",",41",",9",
  "#","23,19","8","9","6","#","16,14","","",
...
function KakuroCell({value}:{value:string}){
  if(value==="#") return <i className="blk"/>;
  if(value.includes(",")){const [across,down]=value.split(",");return <i className="blk clued">{across&&<b>{across}</b>}{down&&<em>{down}</em>}</i>}
  return <i className={value?"given":""}>{value}</i>;
}
...
function GamePreview({game,compact=false}:{game:(typeof games)[number];compact?:boolean}){
  ...
  if(game.id==="kakuro") return <div className={`real-preview ${compact?"compact":""}`} aria-hidden="true"><div className="preview-label"><span>KA1-5</span><b>★☆☆☆☆☆</b></div><div className="real-board kakuro-real">{kakuroBoard.map((value,i)=><KakuroCell key={i} value={value}/>)}</div><div className="preview-bottom"><span>Table</span><span>Hint</span><span>Notes</span></div></div>;
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero">
      ...
      <div className="game-stack" aria-label="Preview of the five games">
    ...
    <footer>
      <div className="footer-main"><a className="wordmark" href="#top" aria-label="DanDoku home"><span className="word-dan">Dan</span><span className="word-doku">Doku</span></a><p>Classic · Variants · Killer Sudoku · Solduku · Kakuro</p><span suppressHydrationWarning>© {new Date().getFullYear()}</span></div>
